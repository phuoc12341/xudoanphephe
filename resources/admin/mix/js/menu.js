...
const Menu = {
    async delete() {
        $('.delete-menu').on('click', async function () {
            let result = confirm('Bạn có chắc chắn xóa menu này ?');
            if (result == false) {
                return false
            }
            ...
            let id = $(this).data('id');
            console.log(route)
            console.log(getRoute(route.menus.destroy, [id]))
            try {
                const response = await ApiServiceDelete(getRoute(route.menus.destroy, [id]))
                ...
                notify('error', 'Không thể xóa được menu này')
            }
        });
    },
